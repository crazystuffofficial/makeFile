...
function makeFiles(name, contenttype, content){
  if(name.length == contenttype.length == content.length){
    for(var i = 0; i < name.length; i++){
      makeFile(name[i], contenttype[i], content[i]);
    }
  } else{
    console.error("Error: params not matching")
  }
}  
